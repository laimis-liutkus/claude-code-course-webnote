...
tools: WebFetch, WebSearch, Skill, ToolSearch, mcp__context7__resolve-library-id, mcp__context7__query-docs
model: sonnet
---
...
If these tools aren't loaded, load them first with `ToolSearch` (`select:mcp__context7__resolve-library-id,mcp__context7__query-docs`). If Context7 returns an error (e.g. quota exceeded), go straight to Step 2.
...
4. **Final fallback - fetch normal page:**
   - If no llms.txt or .md found, WebFetch the official docs page with a prompt targeting the needed info
