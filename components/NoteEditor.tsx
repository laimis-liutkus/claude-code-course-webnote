...
function Separator(): JSX.Element {
  return <span role="separator" aria-orientation="vertical" className="mx-1 h-5 w-px bg-neutral-300 dark:bg-neutral-700" />;
}
...
function Toolbar({ editor }: { editor: Editor }): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <Separator />
      <ToolbarButton label="Paragraph" pressed={state?.paragraph} onPress={run((c) => c.setParagraph())}>
        P
      </ToolbarButton>
      <ToolbarButton label="Heading 1" pressed={state?.h1} onPress={run((c) => c.toggleHeading({ level: 1 }))}>
        H1
      </ToolbarButton>
      <ToolbarButton label="Heading 2" pressed={state?.h2} onPress={run((c) => c.toggleHeading({ level: 2 }))}>
        H2
      </ToolbarButton>
      <ToolbarButton label="Heading 3" pressed={state?.h3} onPress={run((c) => c.toggleHeading({ level: 3 }))}>
        H3
      </ToolbarButton>
      <Separator />
      <ToolbarButton label="Bullet list" pressed={state?.bulletList} onPress={run((c) => c.toggleBulletList())}>
        • List
      </ToolbarButton>
      <ToolbarButton label="Inline code" pressed={state?.code} onPress={run((c) => c.toggleCode())}>
        <span className="font-mono">{"<>"}</span>
      </ToolbarButton>
      <ToolbarButton label="Code block" pressed={state?.codeBlock} onPress={run((c) => c.toggleCodeBlock())}>
        <span className="font-mono">{"{ }"}</span>
      </ToolbarButton>
...
export function NoteEditor({ initialContent, onChange, labelledBy, describedBy }: NoteEditorProps): JSX.Element {
  ...
  return (
    <div className="overflow-hidden rounded-md border border-neutral-300 focus-within:border-neutral-900 focus-within:ring-2 focus-within:ring-neutral-900/20 dark:border-neutral-700 dark:focus-within:border-neutral-100 dark:focus-within:ring-neutral-100/20">
      {editor ? <Toolbar editor={editor} /> : <div aria-hidden="true" className="h-10 border-b border-neutral-200 bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-900" />}
